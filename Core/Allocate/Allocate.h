...
namespace Online::Core
{
    ...
    inline [[nodiscard]] T* New(Args&&... args)
    {
        ...
        void* memory = overAligned ?
            Online::Core::AllocateAligned(sizeof(T), alignof(T)) :
            Online::Core::Allocate(sizeof(T));
        ...
        catch (...)
        {
            ...
            {
                Online::Core::FreeAligned(memory, alignof(T));
            }
            else
            {
                Online::Core::Free(memory);
            }
            throw;
        }
    }
    ...
    inline void Delete(T* ptr) noexcept
    {
        ...
        if constexpr (overAligned)
        {
            Online::Core::FreeAligned(rawMemory, alignof(T));
        }
        else
        {
            Online::Core::Free(rawMemory);
        }
    }
}

#define ONLINE_ALLOW_ALLOCATE                       \
    template<typename U, typename... UArgs>         \
    friend U* Online::Core::New(UArgs&&...);        \
    template<typename U>                            \
    friend void Online::Core::Delete(U*) noexcept;


#define ONLINE_NEW(Type, ...)                                                           \
    [&]() -> Type*                                                                      \
    {                                                                                   \
        constexpr bool overAligned = alignof(Type) > alignof(std::max_align_t);         \
        void* memory = overAligned ?                                                    \
            Online::Core::AllocateAligned(sizeof(Type), alignof(Type)) :                \
            Online::Core::Allocate(sizeof(Type));                                       \
                                                                                        \
        if (!memory) { return nullptr; }                                                \
                                                                                        \
        try                                                                             \
        ...
        {                                                                               \
            if constexpr (overAligned)                                                  \
            {                                                                           \
                Online::Core::FreeAligned(memory, alignof(Type));                       \
            }                                                                           \
            else                                                                        \
            {                                                                           \
                Online::Core::Free(memory);                                             \
