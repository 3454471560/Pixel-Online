...
namespace Online::Log
{
	...
	public:
		struct Factory
		{
			friend class Online::Runtime::Module<Logger>;
		private:
			static Logger* Create()
			{
				return ONLINE_NEW(Logger);
			}
			static void Destroy(Logger* logger)
			{
				ONLINE_DELETE(logger);
			}
		};
		struct Lifecycle
		{
	...
	public:
		void Write(Online::Log::LogLevel level, std::string_view message);

	private:
		inline static void BootstrapLogThread(void* log, void*)
		{
			static_cast<Online::Log::Logger*>(log)->LogThread();
		}
		void LogThread();
	...
	private:
		...
		Online::Core::Thread::Identifier logThread;
